...
FROM prodsys2_repair_management_tbl WHERE sn LIKE '" & TextBox1.Text & "'"
    ...
    Sub NomorOtomatis()
        conn.Open()
        cmd = New MySql.Data.MySqlClient.MySqlCommand("SELECT txn_id FROM prodsys2_labelbox_cskd_printed_tbl WHERE txn_id IN (SELECT MAX(txn_id) FROM prodsys2_labelbox_cskd_printed_tbl)", conn)
        Dim UrutanKode As String
        Dim Hitung As Long
        rd = cmd.ExecuteReader
        rd.Read()
        If Not rd.HasRows Then
            UrutanKode = System.DateTime.Now.ToString("yyMMddHHmmss")
        Else
